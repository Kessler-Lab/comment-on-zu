...
To run these analyses, download supplementary files from Zu et al 2020:
...
- You can also run the code below (set eval=T) to download and unzip the zenodo files, since they are not behind a paywall.
...
#PV_obs <- c(PV_obs, PV_2018)
#AP_obs <- c(AP_obs,AP_2018)

x <- lapply(PV_obs, function(PV) data.frame(species=PV[,1], HPV=specific_information(PV[,-1])))
...
y <- lapply(AP_obs, function(AP) data.frame(species=colnames(AP[,-1]), n=colSums(AP[,-1])))
...
m1 <- lm(n ~ HPV + as.factor(year), data=df)

summary(aov(m))

ss <- summary(aov(m))[[1]][,2]

ss[1]/sum(ss)
# very little variation is explained by specific conditional entropy

ggplot(df, aes(x=HPV, y=n, color=year)) +
  geom_point(size=1)
```
